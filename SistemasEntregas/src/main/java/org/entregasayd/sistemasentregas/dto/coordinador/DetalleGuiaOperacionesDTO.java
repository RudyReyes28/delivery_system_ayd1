package org.entregasayd.sistemasentregas.dto.coordinador;

import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;
import org.entregasayd.sistemasentregas.dto.guias.GetDetalleGuiaDTO;
import org.entregasayd.sistemasentregas.dto.guias.GuiaDetalleClienteDTO;

import java.util.ArrayList;

@Data
@NoArgsConstructor
@AllArgsConstructor
@Builder
public class DetalleGuiaOperacionesDTO {
    //Detalles de la guia
    private GuiaCoordinadorDTO guia;


    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
    public static class GuiaCoordinadorDTO {
        private Long idGuia;
        private String numeroGuia;
        private String codigoInterno;
        private String descripcionContenido;
        private Double valorDeclarado;
        private Double pesoKg;
        private String dimensiones;
        private String observaciones;
        private String fechaCreacion;
        private String fechaRecoleccionReal;
        private String prioridad;
        private Integer intentosEntrega;
        private Double total;
        private String estadoActual;

        //Detalles del cliente
        private GetDetalleGuiaDTO.ClienteDTO cliente;
        //Detalles de la direccion de entrega
        private GetDetalleGuiaDTO.DireccionEntregaDTO direccionEntrega;

        //Listado de estados de la guia
        private ArrayList<GetDetalleGuiaDTO.GuiaEstadoDTO> estadosEntregas;

        private GuiaDetalleClienteDTO.RepartidorDTO repartidor;
    }
}
